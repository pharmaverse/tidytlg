...
derive_freq <- function(df, denom_df, colvar, rowbyvar, tablebyvar, rowvar,
  ...
  df %>%
    mutate(
      across(
        .cols = any_of(c(rowvar, rowbyvar, tablebyvar)),
        .fns = ~ unclass(.),
        .names = "{.col}_ord"
      )
    )
...
pivot_freq <- function(df, colvar, rowvar, rowbyvar, tablebyvar, statlist,
  ...
  res %>%
    mutate(
      ...
      across(
        .cols = -any_of(c("nested_level", ord_columns)),
...
rowbyvar_merge <- function(df, colvar, rowvar, rowbyvar, tablebyvar) {
  if (is.null(rowbyvar)) {
    ...
    df %>%
      var_merge(
        nestby = c(colvar, rowbyvar[1], tablebyvar),
        rowvar = rowvar,
...
var_merge <- function(df, nestby, rowvar, rowvar_name, result_value,
  ...
  ord_values_fill <- setNames(
    as.list(rep(0, length(ord_values_fill))),
    ord_values_fill
  )

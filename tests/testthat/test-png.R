...
test_that("gentlg will preserve aspect ratio of pngs", {
  ...
  withr::with_options(list(tidytlg.add_datetime = FALSE), {
    withr::with_dir(test_path("test_outputs"), {
      gentlg(plotnames = "png1.png", tlf = "f", file = "png1")
      gentlg(plotnames = "png2.png", tlf = "f", file = "png2")
    })
  })
...
test_that("gentlg can take multiple plotnames arguments", {
  ...
  plot1 <- ggplot(data = adsl, aes(x = HEIGHTBL, y = WEIGHTBL)) +
    geom_point() +
    labs(x = "Baseline Height (cm)", y = "Baseline Weight (kg)")
  ...
  plot2 <- ggplot(data = adsl, aes(x = WEIGHTBL, y = HEIGHTBL)) +
    geom_point() +
    labs(x = "Baseline Weight (kg)", y = "Baseline Height (cm)")

...
test_that("T1.3 blanks in place of NA for factor.", {
  ...
  expect_equal(
    as.character(replace_na_with_blank(v)),
    c("", "test 1", "test 2", "test 3")
  )
})
...
test_that("T1.4 NA added as a factor level for factor.", {
  ...
  expect_equal(
    levels(replace_na_with_blank(v)),
    c("", "test 1", "test 2", "test 3")
  )
})
...
test_that("T1.1 label preserved.", {
  expect_equal(
    attr(char2factor(df_char2factor, "TRT01P", "TRT01PN"), "label"),
    "Planned Treatment"
  )
})
...
test_that("T1.2 levels based off the numeric variable.", {
  expect_equal(
    levels(char2factor(df_char2factor, "TRT01P", "TRT01PN")),
